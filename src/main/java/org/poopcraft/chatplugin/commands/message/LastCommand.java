...
import org.poopcraft.chatplugin.utils.MessageUtil;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        for (Map.Entry<UUID, UUID> entry : MessageUtil.getReplyMap().entrySet()) {
            if (entry.getKey() == player.getUniqueId()) {
                target = Bukkit.getPlayer(entry.getValue());
            }
        }

        if (target == null) {
            player.sendMessage(ChatColor.DARK_RED + "You haven't messaged anyone");
            return true;
